...
def _composition(spec: dict[str, Any], video_script: dict[str, Any], audio_duration: float) -> str:
    ...
    for index, segment in enumerate(video_script["segments"]):
        ...
        cards = "".join(
            f'<div class="card {"orange" if item_index == 0 else html.escape(palette[index])}"><span>{html.escape(item["label"])}</span><p>Controls: {html.escape(" · ".join(item["teaching"]["controls"][:2]))}</p></div>'
