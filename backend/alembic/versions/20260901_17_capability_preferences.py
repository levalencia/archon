...
def downgrade() -> None:
    ...
    if dialect == "sqlite":
        ...
        op.execute("DROP TRIGGER IF EXISTS trg_skill_revisions_immutable ON skill_revisions")
        op.execute(
            "CREATE TRIGGER trg_skill_revisions_immutable BEFORE UPDATE OR DELETE ON skill_revisions FOR EACH ROW EXECUTE FUNCTION cogentrex_spi_reject_revision_mutation()"
        )
