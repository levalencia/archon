...
_RECORDED_EVAL_GUIDANCE = (
    "Live fabricated evaluations are retired. Record real runs, then POST /api/evals/runs."
)


@router.post("/ab-test", status_code=410)
async def deprecated_ab_test() -> None:
    """Direct callers to the recorded-run evaluation API."""
    raise HTTPException(status_code=410, detail=_RECORDED_EVAL_GUIDANCE)


@router.post("/harness", status_code=410)
async def deprecated_eval_harness() -> None:
    """Direct callers to the recorded-run evaluation API."""
    raise HTTPException(status_code=410, detail=_RECORDED_EVAL_GUIDANCE)
