...
def test_loads_canonical_root_to_leaf_with_typed_canonical_sources(tmp_path: Path) -> None:
    _write(tmp_path / ".cogentrex/instructions.md", "root")
    _write(tmp_path / "src/.cogentrex/instructions.md", "src")
    _write(tmp_path / "src/api/.cogentrex/instructions.md", "api")
    ...
    assert all(item.family is InstructionFamily.COGENTREX for item in loaded)
    assert [item.relative_path for item in loaded] == [
        ".cogentrex/instructions.md",
        "src/.cogentrex/instructions.md",
        "src/api/.cogentrex/instructions.md",
    ]
...
def test_configured_family_never_merges_other_ecosystems(tmp_path: Path) -> None:
    _write(tmp_path / ".cogentrex/instructions.md", "cogentrex")
    _write(tmp_path / "AGENTS.md", "agents")
    _write(tmp_path / "CLAUDE.md", "claude")

    assert [x.content for x in load_project_instructions(tmp_path, ".")] == ["cogentrex"]
...
def test_imports_are_bounded_ordered_and_cycles_rejected(tmp_path: Path) -> None:
    _write(tmp_path / ".cogentrex/instructions.md", "before\n@include ../shared/base.md\nafter")
...
def test_rejects_target_and_import_path_escape(tmp_path: Path) -> None:
    ...
    _write(tmp_path / ".cogentrex/instructions.md", "@include ../../outside.md")
    with pytest.raises(InstructionLoadError, match="escapes workspace"):
        load_project_instructions(tmp_path, ".")
...
def test_rejects_symlink_escape_and_limits(tmp_path: Path) -> None:
    ...
    (tmp_path / ".cogentrex").mkdir()
    (tmp_path / ".cogentrex/instructions.md").symlink_to(outside)
    with pytest.raises(InstructionLoadError, match="symlink|escapes"):
        load_project_instructions(tmp_path, ".")

    (tmp_path / ".cogentrex/instructions.md").unlink()
    _write(tmp_path / ".cogentrex/instructions.md", "12345")
...
def test_enforces_file_depth_total_bytes_and_directory_depth(tmp_path: Path) -> None:
    _write(tmp_path / ".cogentrex/instructions.md", "@include a.md")
    _write(tmp_path / ".cogentrex/a.md", "@include b.md")
    _write(tmp_path / ".cogentrex/b.md", "b")
...
def test_rejects_hardlinked_instruction_source(tmp_path: Path) -> None:
    ...
    target = tmp_path / ".cogentrex/instructions.md"
    target.parent.mkdir()
...
def test_fails_closed_without_descriptor_relative_traversal(
    ...
    _write(tmp_path / ".cogentrex/instructions.md", "safe")
