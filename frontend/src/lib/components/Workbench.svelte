<script lang="ts">
 ...
 onMount(() => { hydrated = true; loadHealth(); connectLogs(); if (initialId) loadConversation(initialId, false); return () => { controller?.abort(); logSource?.close(); }; });
 ...
 function connectLogs() { logSource = new EventSource('/api/logs/stream'); logSource.onmessage = e => { try { logs = [...logs.slice(-249), JSON.parse(e.data)]; } catch {} }; }
 ...
 async function loadConversation(id: string, route=true) { controller?.abort(); currentId=id; setOverlay(sidebarElement, sidebarScrim, false); error=''; loading=true; if(route) goto(`/chat/${id}`); try { const r=await fetch(`/api/chat/history/${id}`); if(!r.ok) throw new Error(`Could not load conversation (${r.status})`); const d=await r.json(); messages=(d.messages||[]).map((m:any,i:number)=>({id:i,role:m.role,content:m.content,timestamp:''})); } catch(e) { error=e instanceof Error?e.message:'Could not load conversation'; messages=[]; } finally { loading=false; } }
