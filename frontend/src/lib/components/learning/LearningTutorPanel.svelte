<script lang="ts">
  ...
  let threadElement = $state<HTMLElement | undefined>(undefined);
  ...
  async function submit(value = question) {
    ...
    progressMessage = '';
    streamingText = '';
    const requestContextKey = key(context);
    ...
    try {
      await streamLearningTutor(clean, context, {
        ...
        onStatus(data) {
          if (activeController !== controller || key(context) !== requestContextKey) return;
        ...
        onResult(data) {
          ...
          if (typeof localStorage !== 'undefined') {
            localStorage.setItem(`cogentrex_learning_tutor:${requestContextKey}`, data.session_id);
          }
          void scrollToLatest();
        },
        onError(data) {
        ...
        onDone() {
          if (activeController === controller) loading = false;
        },
      });
    } catch (cause) {
  ...
  $effect(() => {
    const currentKey = key(context);
    if (activeContextKey && currentKey !== activeContextKey) cancelActiveRequest();
...
{#if open}
  <div class="backdrop" role="presentation" onclick={(event) => { if (event.currentTarget === event.target) closePanel(); }}>
    <aside class="panel" aria-label="Learning tutor">
      <header>
        ...
        <button class="icon" aria-label="Close learning tutor" onclick={closePanel}><X size={18}/></button>
      </header>
      <div class="thread" bind:this={threadElement} aria-live="polite">
        {#if !answers.length}
        ...
        {/each}
        {#if loading}<div class="loading" role="status"><span>{progressMessage || 'Retrieving and verifying evidence…'}</span><button type="button" onclick={cancelActiveRequest}>Cancel</button></div>{/if}
...
{/if}
<svelte:window onkeydown={(event) => { if (open && event.key === 'Escape') closePanel(); }}/>
