<script lang="ts">
  ...
  const sectionColors = ['#f6b44b', '#22c55e', '#6ee7ff', '#a78bfa', '#fb7185'];
  const rules = [
...
<article class="infographic" aria-labelledby="infographic-title">
  <header class="hero">
    <div>
      <span class="kicker">Cogentrex evidence engineering</span>
  ...
  <div class="editorial-grid">
    ...
    <section class="module architecture" aria-labelledby="architecture-title">
      ...
      <div class="architecture-body">
        <button class="claim-box interactive" class:active={selectedModuleId === 'capability-claim'} onclick={() => selectModule('capability-claim')} aria-pressed={selectedModuleId === 'capability-claim'}><span>Capability claim</span><strong>What are we saying Cogentrex can do?</strong></button>
    ...
    <section class="module journey" aria-labelledby="journey-title">
      ...
      <div class="journey-track" aria-label="Five ordered evidence levels">
        {#each content.nodes as node, index}
          ...
          <button class:active={selected === index} style={`--level-color:${sectionColors[index] ?? '#f6b44b'}`} onclick={() => selectLevel(index)} aria-pressed={selected === index}>
...
<style>
  .infographic{border:1px solid var(--cogentrex-border,var(--border));border-radius:1rem;padding:clamp(1rem,2.5vw,1.75rem);background:var(--cogentrex-canvas,#050712);color:var(--cogentrex-text,#f4f7fb)}
  .hero{display:flex;justify-content:space-between;gap:1.5rem;align-items:center;padding:1rem 1.15rem 1.25rem;border-bottom:5px solid var(--cogentrex-orange,#f6b44b);background:var(--cogentrex-surface,#0a1022);border-radius:.8rem .8rem .25rem .25rem}
  .kicker{color:var(--cogentrex-orange,#f6b44b);font:800 .68rem var(--font-mono);letter-spacing:.14em;text-transform:uppercase}.hero h3{margin:.35rem 0;font-size:clamp(1.8rem,4vw,3.2rem);line-height:1;color:#f4f7fb}.hero p{margin:.55rem 0 0;max-width:760px;color:#cbd5e1;line-height:1.55}.hero-mark{display:grid;place-items:center;min-width:125px;padding:.8rem;border:2px solid var(--cogentrex-orange,#f6b44b);border-radius:.75rem;color:#f6b44b;box-shadow:0 0 24px var(--cogentrex-orange-glow,rgba(246,180,75,.22))}.hero-mark strong{font-size:1rem;letter-spacing:.12em}.hero-mark span{font-size:.68rem;color:#a9b4cc}
  .editorial-grid{display:grid;grid-template-columns:minmax(220px,.72fr) minmax(0,1.6fr);gap:.85rem;margin-top:.85rem}.module{overflow:hidden;border:1px solid #26324d;border-radius:.75rem;background:#0a1022;box-shadow:0 8px 24px rgba(0,0,0,.28)}.module-title{display:flex;align-items:center;gap:.5rem;padding:.65rem .8rem;color:white}.module-title h4{margin:0;font-size:.82rem;letter-spacing:.05em;text-transform:uppercase}.amber{background:#dc8b18}.green{background:#15803d}.blue{background:#2563eb}.purple{background:#7c3aed}.coral{background:#e11d48}
  .module-body{display:grid;gap:.35rem;padding:.7rem}.interactive{font:inherit;color:inherit;cursor:pointer;text-align:left;transition:border-color .16s ease,box-shadow .16s ease,transform .16s ease}.interactive:focus-visible{outline:2px solid var(--cogentrex-orange,#f6b44b);outline-offset:2px}.interactive:focus-visible,.interactive.active{border-color:var(--cogentrex-orange,#f6b44b)!important;box-shadow:0 0 0 1px rgba(246,180,75,.45),0 0 24px var(--cogentrex-orange-glow,rgba(246,180,75,.22))!important}.rule{display:grid;width:100%;grid-template-columns:30px 1fr;gap:.55rem;padding:.6rem;border:0;border-bottom:1px solid #263449;background:transparent;color:#e2e8f0}.rule:last-child{border-bottom:0}.rule :global(svg){color:#f6b44b}.rule span{display:block}.rule strong,.impact-grid strong{font-size:.76rem;text-transform:uppercase}.rule small{display:block;margin-top:.15rem;color:#aebdd0;font-size:.7rem;line-height:1.45}
  .architecture-body{display:grid;grid-template-columns:1fr auto 1.05fr auto 1fr;align-items:center;gap:.65rem;padding:1rem}.claim-box,.verdict-box{min-height:105px;display:flex;flex-direction:column;justify-content:center;padding:.8rem;border-radius:.65rem;color:#f4f7fb}.claim-box{border:2px solid #60a5fa;background:#0b2447}.verdict-box{border:2px solid #4ade80;background:#0b2d24}.claim-box span,.verdict-box span{font:800 .62rem var(--font-mono);text-transform:uppercase;color:#b8c5d6}.claim-box strong,.verdict-box strong{margin-top:.35rem;font-size:.8rem;line-height:1.35}.proof-stack{display:grid;gap:.35rem}.proof-stack button{display:block;width:100%;padding:.45rem .55rem;border:1px solid #2d7d54;border-radius:.4rem;background:#0b2d24;text-align:left}.proof-stack span{display:block;color:#86efac;font-size:.72rem;font-weight:800}.proof-stack small{color:#b7c7d8;font-size:.6rem}.architecture-body :global(.flow-arrow){color:#22c55e}.cross-cutting{display:flex;justify-content:center;align-items:center;gap:.5rem;margin:0 .8rem .8rem;padding:.55rem;border:1px dashed #4ade80;border-radius:.45rem;background:#0b2d24;color:#bbf7d0;font-size:.68rem;font-weight:700}
  .journey{grid-column:1/-1}.journey-track{display:flex;align-items:center;justify-content:center;gap:.35rem;padding:1rem}.journey-track button{position:relative;flex:1;min-width:0;min-height:115px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:.35rem;border:2px solid color-mix(in srgb,var(--level-color) 70%,#26324d);border-radius:.65rem;background:color-mix(in srgb,var(--level-color) 14%,#0a1022);color:#f4f7fb;padding:.65rem;text-align:center}.journey-track button.active,.journey-track button:hover,.journey-track button:focus-visible{border-color:var(--level-color);box-shadow:0 0 0 3px color-mix(in srgb,var(--level-color) 20%,transparent);transform:translateY(-2px)}.journey-track button :global(svg){color:var(--level-color)}.journey-track strong{font-size:.72rem;line-height:1.25}.journey-track small{color:#b8c5d6;font-size:.61rem}.step{position:absolute;top:-9px;left:-9px;display:grid;place-items:center;width:23px;height:23px;border-radius:50%;background:var(--level-color);color:white;font:800 .65rem var(--font-mono)}.journey-track :global(.journey-arrow){flex:0 0 auto;color:#64748b}.infographic-inspector{margin:0 1rem 1rem}
  .why,.checklist{min-height:225px}.impact-grid{display:grid;grid-template-columns:1fr 1fr;gap:.5rem;padding:.8rem}.impact-grid button{padding:.65rem;border:0;border-left:3px solid #a78bfa;background:#11182d;color:#f4f7fb;text-align:left}.impact-grid p{margin:.15rem 0 0;color:#aebdd0;font-size:.7rem;line-height:1.45}.checklist ul{display:grid;gap:.3rem;list-style:none;margin:0;padding:.8rem}.checklist li{display:flex;align-items:center;gap:.55rem;padding:.45rem;border-bottom:1px solid #263449;font-size:.72rem;font-weight:700;color:#e2e8f0}.checklist li:last-child{border:0}.checklist li span{color:#fb7185;font:800 .63rem var(--font-mono)}
  .principle{display:flex;align-items:center;gap:.75rem;margin-top:.85rem;padding:.8rem 1rem;border-radius:.65rem;background:#172033;color:#fff}.principle :global(svg){flex:none;color:#f6b44b}.principle span{display:block;color:#fbbf24;font:800 .62rem var(--font-mono);text-transform:uppercase}.principle strong{font-size:.78rem;line-height:1.45}
