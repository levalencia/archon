<script lang="ts">
 ...
 const label=(item:EffectiveContextEntry)=>item.source_path||item.relative_path||item.path||item.name||item.id;
 const revision=(item:EffectiveContextEntry)=>item.revision_id||item.revision||item.version||'';
 const hash=(item:EffectiveContextEntry)=>item.content_hash||item.hash;
 const reason=(item:EffectiveContextEntry)=>item.selection_reason||item.reason||item.reasons?.join(', ')||'Selected by deterministic resolver';
</script>
<div data-testid="effective-context">
 ...
 {:else if manifest}<div class="space-y-3">
  ...
  {#each [{title:'Instruction revisions',items:manifest.instruction_revisions,icon:FileText},{title:'Skill revisions',items:manifest.skill_revisions,icon:Package},{title:'Exposed capabilities',items:manifest.capabilities,icon:Braces}] as group}<details class="min-w-0 rounded-lg border border-[var(--border)] bg-[var(--panel-2)]" open={group.title==='Instruction revisions'}><summary class="flex min-h-11 cursor-pointer list-none items-center gap-2 px-3 text-xs font-semibold"><group.icon size={14} class="text-[var(--accent)]"/>{group.title}<span class="ml-auto text-[var(--muted)]">{group.items.length}</span></summary>{#if !group.items.length}<p class="border-t border-[var(--border)] p-3 text-xs text-[var(--muted)]">None selected.</p>{:else}<ol class="m-0 list-none border-t border-[var(--border)] p-0">{#each group.items as item,index}<li class="min-w-0 border-b border-[var(--border)] p-3 text-xs last:border-0"><div class="flex min-w-0 justify-between gap-2"><strong class="min-w-0 break-all" title={label(item)}><span class="mr-2 text-[var(--muted)]">#{item.resolution_order??item.order??index+1}</span>{label(item)}</strong><span class="shrink-0 font-mono text-[var(--muted)]">{revision(item)}</span></div><p class="my-1 text-[var(--secondary)]">{reason(item)}</p><div class="flex min-w-0 flex-col gap-1 font-mono text-[10px] text-[var(--muted)]">{#if hash(item)}<span class="break-all">content {hash(item)}</span>{/if}{#if item.schema_hash}<span class="break-all">schema {item.schema_hash}</span>{/if}{#if item.permission}<span>policy {item.permission}</span>{/if}{#if item.estimated_tokens!=null}<span>{item.estimated_tokens} tokens</span>{/if}</div></li>{/each}</ol>{/if}</details>{/each}
