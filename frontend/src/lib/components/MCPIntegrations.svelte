...
<section id="mcp" aria-labelledby="mcp-heading" class="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-xl p-4 sm:p-5 space-y-5">
  ...
  <div class="grid sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] gap-2">
    <label class="text-xs text-[var(--text-muted)]">Project
      <input aria-label="Project" bind:value={projectId} onblur={load} class="mt-1 w-full px-3 py-2 rounded-lg bg-[var(--bg-primary)] border border-[var(--border)] text-sm text-[var(--text-primary)]" />
    </label>
    <label class="text-xs text-[var(--text-muted)]">Integration name
      <input aria-label="Integration name" bind:value={name} placeholder="Documentation server" class="mt-1 w-full px-3 py-2 rounded-lg bg-[var(--bg-primary)] border border-[var(--border)] text-sm text-[var(--text-primary)]" />
    </label>
    <label class="text-xs text-[var(--text-muted)] sm:min-w-44">Approved profile
      <select aria-label="Approved profile" bind:value={profileId} disabled={!profiles.length} class="mt-1 w-full px-3 py-2 rounded-lg bg-[var(--bg-primary)] border border-[var(--border)] text-sm text-[var(--text-primary)]">
    ...
    <button onclick={create} disabled={busy === 'create' || !name.trim() || !profileId} class="sm:col-start-3 inline-flex justify-center items-center gap-2 px-4 py-2 rounded-lg bg-[var(--accent)] text-white text-sm disabled:opacity-50">
        ...
        <article data-testid="mcp-server" class="border border-[var(--border)] rounded-lg p-3 sm:p-4 space-y-4">
          <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
            ...
            <div class="flex flex-wrap gap-2">
              <button aria-label={`${server.enabled ? 'Disable' : 'Enable'} ${server.name}`} onclick={() => setServerEnabled(server)} disabled={busy === `server:${server.id}`} class="px-3 py-1.5 border border-[var(--border)] rounded-md text-xs text-[var(--text-secondary)]">{server.enabled ? 'Disable' : 'Enable'}</button>
              <button onclick={() => discover(server)} disabled={!server.enabled || busy === `discover:${server.id}`} class="inline-flex items-center gap-1 px-3 py-1.5 border border-[var(--border)] rounded-md text-xs text-[var(--text-secondary)]"><RefreshCw size={12}/>{busy === `discover:${server.id}` ? 'Discovering…' : 'Discover tools'}</button>
              <button aria-label={`Delete ${server.name}`} onclick={() => remove(server)} disabled={busy === `delete:${server.id}`} class="p-1.5 text-[var(--error)]"><Trash2 size={14}/></button>
            </div>
          </div>
            ...
            {#each tools[server.id] as tool (tool.id)}
              <div data-testid="mcp-tool" class="flex flex-col sm:flex-row sm:items-start justify-between gap-3 bg-[var(--bg-tertiary)] rounded-lg p-3">
                <div class="min-w-0">
                  ...
                  <details class="mt-2"><summary class="text-xs cursor-pointer text-[var(--text-secondary)]">Input schema</summary><pre class="overflow-auto text-[10px] mt-1 p-2 rounded bg-[var(--bg-primary)]">{JSON.stringify(tool.input_schema, null, 2)}</pre></details>
                </div>
                <label class="inline-flex items-center gap-2 text-xs text-[var(--text-secondary)]"><input type="checkbox" checked={tool.enabled} onchange={() => setToolEnabled(server, tool)} disabled={busy === `tool:${tool.id}`} /> Enabled</label>
              </div>
            {/each}
          </div>{/if}
        </article>
