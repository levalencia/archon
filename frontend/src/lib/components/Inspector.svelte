<script lang="ts">
  ...
  const tabs: InspectorTab[] = ['run','agents','evidence','context','logs'];
  let verifierOk = $derived(message?.verifier?.supported !== false && !message?.verifier?.unsupported_claims?.length);
  let runLive = $derived(message?.status === 'streaming');
  let effectiveRunId = $state('');
  ...
  $effect(() => {
    ...
    if ((selectedTab === 'context' || selectedTab === 'agents') && selectedConversation) {
      queueMicrotask(() => void loadContextManifest(selectedConversation));
    }
  });
</script>
<div class="inspector">
  <header><div><p class="eyebrow">Inspector</p><h2>Run evidence</h2></div><span class="live"><i></i> {runLive ? 'Live' : 'Recorded'}</span></header>
  <div class="tabs" role="tablist" aria-label="Inspector views">{#each tabs as tab}<button id={`inspector-tab-${tab}`} aria-controls={`inspector-panel-${tab}`} role="tab" aria-selected={active === tab} onclick={() => onTab(tab)}>{tab[0].toUpperCase()+tab.slice(1)}</button>{/each}</div>
  <div class="inspector-content" id={`inspector-panel-${active}`} role="tabpanel" aria-labelledby={`inspector-tab-${active}`}>
  {#if active === 'run'}
  ...
  {:else if active === 'context'}
    ...
    {#if context?.compacted}<div class="mt-3 rounded-lg border border-[var(--accent)] bg-[var(--accent-glow)] p-3 text-xs text-[var(--accent)]">Context compacted — {context.tokens_before?.toLocaleString()} → {context.tokens_after?.toLocaleString()} tokens ({context.saved_pct}% saved)</div>{/if}
    <p class="section-label mt-4">Effective context</p>
    {#if contextManifestLoading}
      <div class="empty-detail" aria-live="polite">Locating persisted run…</div>
    {:else if contextManifestError}
      <div class="empty-detail" role="status">{contextManifestError}</div>
    {:else if effectiveRunId}
      <EffectiveContext runId={effectiveRunId} />
    {:else}
      <div class="empty-detail">Select a persisted conversation to inspect its effective context.</div>
    {/if}
  {:else}
