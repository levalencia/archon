...
<div class="timeline">
 ...
 {#if runs.length}
  ...
  {#if selected}<RunSummary run={selected} {events} {onFork}/><RunExportPanel runId={selected.run_id}/><details class="effective-ledger"><summary>Effective context</summary><EffectiveContext runId={selected.run_id}/></details>{/if}
