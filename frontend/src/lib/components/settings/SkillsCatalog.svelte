...
<section id="skills" aria-labelledby="skills-title" class="min-w-0 rounded-xl border border-[var(--border)] bg-[var(--panel)] p-4 sm:p-5">
 ...
 <div class="mt-4 grid grid-cols-3 rounded-lg border border-[var(--border)] p-1" role="tablist" aria-label="Skill catalog filters">{#each tabs as tab}<button id={`skills-tab-${tab}`} role="tab" aria-selected={active===tab} aria-controls="skills-panel" tabindex={active===tab?0:-1} class="min-h-11 rounded-md px-2 text-xs capitalize {active===tab?'bg-[var(--accent-glow)] text-[var(--accent)] shadow-[0_0_14px_var(--archon-orange-glow)]':'text-[var(--muted)]'}" onclick={()=>active=tab} onkeydown={onTabKey}>{tab}</button>{/each}</div>
...
<style>
  article span.rounded-full {
    ...
    box-shadow: 0 0 12px var(--archon-orange-glow);
  }
</style>
