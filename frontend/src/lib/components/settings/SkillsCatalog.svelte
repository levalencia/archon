<script lang="ts">
  import { Package, Pin, ToggleLeft, ToggleRight } from 'lucide-svelte';
  ...
  let loading = $state(true); let error = $state(''); let busy = $state('');
  const filtered = $derived(skills.filter((s) => active === 'enabled' ? s.enabled : active === 'available' ? !s.enabled : true));
  async function load() { loading=true; error=''; try { skills=await listSkillCatalog(projectId); } catch(e){ error=e instanceof Error?e.message:'Skills unavailable'; } finally{loading=false;} }
