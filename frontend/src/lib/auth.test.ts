...
describe('authenticatedFetch', () => {
  ...
  it('clears stale authentication after a 401 response', async () => {
    ...
    localStorage.setItem('cogentrex_token', 'stale-token');
    localStorage.setItem('cogentrex_user', JSON.stringify({ user_id: 'u1', username: 'luis' }));
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(null, { status: 401 })));

    const response = await authenticatedFetch('/api/learning-media/catalog');

    expect(response.status).toBe(401);
    expect(localStorage.getItem('cogentrex_token')).toBeNull();
    expect(localStorage.getItem('cogentrex_user')).toBeNull();
  });

  it('keeps valid authentication and sends the bearer token', async () => {
    localStorage.setItem('cogentrex_token', 'valid-token');
