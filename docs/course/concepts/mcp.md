...
> **Status boundary:** Archon wires governed MCP 2.1.1 stdio discovery and invocation through owner scope, policy, approvals, and bounded schemas; HTTP/OAuth parity is not claimed.
...
MCP is a protocol through which a client discovers a server’s tools and invokes them using structured messages. Archon does not trust discovery by default. **It is not:** MCP itself is not authorization, sandboxing, approval, or proof that a tool is safe.
...
MCP is a universal plug shape, while Archon’s inventory and policy are the locked switchboard deciding what can receive power. Inputs cross a validation boundary; outputs are bounded outcomes plus evidence. Important invariants are explicit scope, finite resources, sanitized failures, and no authority inferred from untrusted payloads.
...
flowchart LR
  ...
  V --> C[Model Context Protocol (MCP) boundary]
...
| `MCPRuntimeToolProvider.for_scope` | Implements the central contract. | Production guarantees beyond its wired path. |
...
sequenceDiagram
  ...
  participant Core as MCPRuntimeToolProvider.for_scope
...
classDiagram
  class Boundary
  class Core {
    +MCPRuntimeToolProvider.for_scope()
  }
  class EvidenceSink
  Boundary --> Core
  Core --> EvidenceSink
...
|---|---|---|
| [`backend/app/mcp/runtime.py:MCPRuntimeToolProvider.for_scope`](../../../backend/app/mcp/runtime.py) | Central implemented behavior. | Inspect call sites before extending the claim. |
...
|---|---|---|
| [`backend/tests/integration/test_mcp_runtime.py`](../../../backend/tests/integration/test_mcp_runtime.py) | Deterministic contract for the listed implementation. | External-provider parity, load, public deployment, or SLOs. |
...
```bash
cd backend
uv run pytest -q tests/integration/test_mcp_runtime.py
```

Then locate `MCPRuntimeToolProvider.for_scope` in `backend/app/mcp/runtime.py` and write one sentence naming what the test proves and one sentence naming what it cannot prove.

### Done criteria

- [ ] The focused test passes and its assertion is identified.
- [ ] The result is tied to `MCPRuntimeToolProvider.for_scope`, not merely to a file.
...
> MCP is a protocol through which a client discovers a server’s tools and invokes them using structured messages. Archon does not trust discovery by default. In Archon the boundary is `MCPRuntimeToolProvider.for_scope`; `backend/tests/integration/test_mcp_runtime.py` exercises its contract. The honest limit is: Archon wires governed MCP 2.1.1 stdio discovery and invocation through owner scope, policy, approvals, and bounded schemas; HTTP/OAuth parity is not claimed.
...
1. Define Model Context Protocol (MCP) without naming an Archon class.
...
1. MCP is a protocol through which a client discovers a server’s tools and invokes them using structured messages. Archon does not trust discovery by default.
2. MCP itself is not authorization, sandboxing, approval, or proof that a tool is safe.
...
4. `backend/app/mcp/runtime.py:MCPRuntimeToolProvider.for_scope` and `backend/tests/integration/test_mcp_runtime.py`.
