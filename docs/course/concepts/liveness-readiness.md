# Concept: Liveness and readiness

> **Implementation status:** `implemented`
> **Status boundary:** /healthz reports process liveness; /readyz probes database, rate limiter and active configured OTEL, plus capability metadata. It is local evidence, not an SLA.
...
> **Catalog ID:** `liveness-readiness`

## Beginner explanation

Liveness asks whether a process should be restarted; readiness asks whether it can currently serve traffic with required dependencies. **It is not:** A successful probe does not prove every external provider or business workflow works.
...
Liveness is a pulse; readiness is the checklist before opening the door. Inputs cross a validation boundary; outputs are bounded outcomes plus evidence. Important invariants are explicit scope, finite resources, sanitized failures, and no authority inferred from untrusted payloads.
...
flowchart LR
  ...
  V --> C[Liveness and readiness boundary]
...
| `create_app.readyz` | Implements the central contract. | Production guarantees beyond its wired path. |
...
sequenceDiagram
  ...
  participant Core as create_app.readyz
...
|---|---|---|
| [`backend/app/main.py:create_app.readyz`](../../../backend/app/main.py) | Central implemented behavior. | Inspect call sites before extending the claim. |
...
|---|---|---|
| [`backend/tests/unit/test_health.py`](../../../backend/tests/unit/test_health.py) | Deterministic contract for the listed implementation. | External-provider parity, load, public deployment, or SLOs. |
...
```bash
cd backend
uv run pytest -q tests/unit/test_health.py
```

Then locate `create_app.readyz` in `backend/app/main.py` and write one sentence naming what the test proves and one sentence naming what it cannot prove.

### Done criteria

- [ ] The focused test passes and its assertion is identified.
- [ ] The result is tied to `create_app.readyz`, not merely to a file.
...
> Liveness asks whether a process should be restarted; readiness asks whether it can currently serve traffic with required dependencies. In Archon the boundary is `create_app.readyz`; `backend/tests/unit/test_health.py` exercises its contract. The honest limit is: /healthz reports process liveness; /readyz probes database, rate limiter and active configured OTEL, plus capability metadata. It is local evidence, not an SLA.
...
1. Define Liveness and readiness without naming an Archon class.
...
1. Liveness asks whether a process should be restarted; readiness asks whether it can currently serve traffic with required dependencies.
2. A successful probe does not prove every external provider or business workflow works.
...
4. `backend/app/main.py:create_app.readyz` and `backend/tests/unit/test_health.py`.
