# Concept: Evidence-only verifier child

> **Implementation status:** `implemented`
> **Status boundary:** One verifier child checks only delegated claims against delegated evidence, receives no tools, and returns strict verdicts.
...
> **Catalog ID:** `verifier-child`

## Beginner explanation

The verifier child is a second model call isolated from tools and outside knowledge by contract; it may support, reject, or escalate each claim. **It is not:** It does not generate the answer, verify source truth, or perform generic self-reflection.
...
Treat it as a read-only reviewer with a sealed evidence folder. The prompt helps, but typed validation and no-tools wiring are the enforceable boundaries. Inputs cross a validation boundary; outputs are bounded outcomes plus evidence. Important invariants are explicit scope, finite resources, sanitized failures, and no authority inferred from untrusted payloads.
...
flowchart LR
  ...
  V --> C[Evidence-only verifier child boundary]
...
| `EvidenceVerifierSpecialist.verify` | Implements the central contract. | Production guarantees beyond its wired path. |
...
sequenceDiagram
  ...
  participant Core as EvidenceVerifierSpecialist.verify
...
classDiagram
  class Boundary
  class Core {
    +EvidenceVerifierSpecialist.verify()
  }
  class EvidenceSink
  Boundary --> Core
  Core --> EvidenceSink
...
|---|---|---|
| [`backend/app/delegation/service.py:EvidenceVerifierSpecialist.verify`](../../../backend/app/delegation/service.py) | Central implemented behavior. | Inspect call sites before extending the claim. |
...
|---|---|---|
| [`backend/tests/unit/test_evidence_verifier.py`](../../../backend/tests/unit/test_evidence_verifier.py) | Deterministic contract for the listed implementation. | External-provider parity, load, public deployment, or SLOs. |
...
```bash
cd backend
uv run pytest -q tests/unit/test_evidence_verifier.py
```

Then locate `EvidenceVerifierSpecialist.verify` in `backend/app/delegation/service.py` and write one sentence naming what the test proves and one sentence naming what it cannot prove.

### Done criteria

- [ ] The focused test passes and its assertion is identified.
- [ ] The result is tied to `EvidenceVerifierSpecialist.verify`, not merely to a file.
...
> The verifier child is a second model call isolated from tools and outside knowledge by contract; it may support, reject, or escalate each claim. In Archon the boundary is `EvidenceVerifierSpecialist.verify`; `backend/tests/unit/test_evidence_verifier.py` exercises its contract. The honest limit is: One verifier child checks only delegated claims against delegated evidence, receives no tools, and returns strict verdicts.
...
1. Define Evidence-only verifier child without naming an Archon class.
...
1. The verifier child is a second model call isolated from tools and outside knowledge by contract; it may support, reject, or escalate each claim.
2. It does not generate the answer, verify source truth, or perform generic self-reflection.
...
4. `backend/app/delegation/service.py:EvidenceVerifierSpecialist.verify` and `backend/tests/unit/test_evidence_verifier.py`.
