...
> **Status boundary:** Persistent users authenticate by scrypt password plus HS256 JWT/cookie or hashed API key; this local design has no external IdP or key-rotation proof.
...
> **Catalog ID:** `authentication`

## Beginner explanation

Authentication answers “who is calling?” Archon resolves a credential to a current durable user before protected routes execute. **It is not:** Authentication does not decide whether the user owns a particular run or may perform a tool action.
...
A credential is a badge checked against the current roster; possessing an old claim is insufficient if the user no longer exists. Inputs cross a validation boundary; outputs are bounded outcomes plus evidence. Important invariants are explicit scope, finite resources, sanitized failures, and no authority inferred from untrusted payloads.
...
| `get_current_user` | Implements the central contract. | Production guarantees beyond its wired path. |
...
sequenceDiagram
  ...
  participant Core as get_current_user
...
|---|---|---|
| [`backend/app/security/auth.py:get_current_user`](../../../backend/app/security/auth.py) | Central implemented behavior. | Inspect call sites before extending the claim. |
...
|---|---|---|
| [`backend/tests/integration/test_auth_persistence.py`](../../../backend/tests/integration/test_auth_persistence.py) | Deterministic contract for the listed implementation. | External-provider parity, load, public deployment, or SLOs. |
...
```bash
cd backend
uv run pytest -q tests/integration/test_auth_persistence.py
```

Then locate `get_current_user` in `backend/app/security/auth.py` and write one sentence naming what the test proves and one sentence naming what it cannot prove.

### Done criteria

- [ ] The focused test passes and its assertion is identified.
- [ ] The result is tied to `get_current_user`, not merely to a file.
...
> Authentication answers “who is calling?” Archon resolves a credential to a current durable user before protected routes execute. In Archon the boundary is `get_current_user`; `backend/tests/integration/test_auth_persistence.py` exercises its contract. The honest limit is: Persistent users authenticate by scrypt password plus HS256 JWT/cookie or hashed API key; this local design has no external IdP or key-rotation proof.
...
1. Define Authentication without naming an Archon class.
...
1. Authentication answers “who is calling?” Archon resolves a credential to a current durable user before protected routes execute.
2. Authentication does not decide whether the user owns a particular run or may perform a tool action.
...
4. `backend/app/security/auth.py:get_current_user` and `backend/tests/integration/test_auth_persistence.py`.
