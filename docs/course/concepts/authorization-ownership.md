# Concept: Authorization and ownership

> **Implementation status:** `implemented`
> **Status boundary:** Protected resources and MCP bindings are filtered by authenticated owner and project where applicable; policy/approval adds action authorization.
...
> **Catalog ID:** `authorization-ownership`

## Beginner explanation

Authorization answers “may this identity do this?” Ownership makes resource lookup include the authenticated user rather than trusting a user ID from input. **It is not:** A 401 identity failure, 403 privilege failure, 404 owner-scoped miss, and tool-policy denial are distinct.
...
The object key is effectively (owner, project, id), not id alone. Inputs cross a validation boundary; outputs are bounded outcomes plus evidence. Important invariants are explicit scope, finite resources, sanitized failures, and no authority inferred from untrusted payloads.
...
flowchart LR
  ...
  V --> C[Authorization and ownership boundary]
...
|---|---|---|
| [`backend/tests/security/test_conversation_ownership.py`](../../../backend/tests/security/test_conversation_ownership.py) | Deterministic contract for the listed implementation. | External-provider parity, load, public deployment, or SLOs. |
...
```bash
cd backend
uv run pytest -q tests/security/test_conversation_ownership.py
```
...
> Authorization answers “may this identity do this?” Ownership makes resource lookup include the authenticated user rather than trusting a user ID from input. In Archon the boundary is `chat_stream_real`; `backend/tests/security/test_conversation_ownership.py` exercises its contract. The honest limit is: Protected resources and MCP bindings are filtered by authenticated owner and project where applicable; policy/approval adds action authorization.
...
1. Define Authorization and ownership without naming an Archon class.
...
1. Authorization answers “may this identity do this?” Ownership makes resource lookup include the authenticated user rather than trusting a user ID from input.
2. A 401 identity failure, 403 privilege failure, 404 owner-scoped miss, and tool-policy denial are distinct.
...
4. `backend/app/routes/stream.py:chat_stream_real` and `backend/tests/security/test_conversation_ownership.py`.
