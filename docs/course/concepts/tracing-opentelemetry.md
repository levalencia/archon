# Concept: Tracing and OpenTelemetry

> **Implementation status:** `implemented`
> **Status boundary:** Runtime spans are exported through the real OTLP SDK when configured; a local collector observed agent.run, but no hosted backend or production sampling was verified.
...
> **Catalog ID:** `tracing-opentelemetry`

## Beginner explanation

A trace connects timed spans for one request so operators can see where time and failures occurred. OpenTelemetry supplies standard APIs/export. **It is not:** Traces do not expose chain-of-thought, prove semantic correctness, or replace logs/metrics.
...
A trace is a request journey map linked by run and correlation identifiers. Inputs cross a validation boundary; outputs are bounded outcomes plus evidence. Important invariants are explicit scope, finite resources, sanitized failures, and no authority inferred from untrusted payloads.
...
flowchart LR
  ...
  V --> C[Tracing and OpenTelemetry boundary]
...
| `CompositeEventSink.emit` | Implements the central contract. | Production guarantees beyond its wired path. |
...
sequenceDiagram
  ...
  participant Core as CompositeEventSink.emit
...
|---|---|---|
| [`backend/app/observability/runtime_events.py:CompositeEventSink.emit`](../../../backend/app/observability/runtime_events.py) | Central implemented behavior. | Inspect call sites before extending the claim. |
...
|---|---|---|
| [`backend/tests/unit/test_otel_tracing_wire.py`](../../../backend/tests/unit/test_otel_tracing_wire.py) | Deterministic contract for the listed implementation. | External-provider parity, load, public deployment, or SLOs. |
...
```bash
cd backend
uv run pytest -q tests/unit/test_otel_tracing_wire.py
```

Then locate `CompositeEventSink.emit` in `backend/app/observability/runtime_events.py` and write one sentence naming what the test proves and one sentence naming what it cannot prove.

### Done criteria

- [ ] The focused test passes and its assertion is identified.
- [ ] The result is tied to `CompositeEventSink.emit`, not merely to a file.
...
> A trace connects timed spans for one request so operators can see where time and failures occurred. OpenTelemetry supplies standard APIs/export. In Archon the boundary is `CompositeEventSink.emit`; `backend/tests/unit/test_otel_tracing_wire.py` exercises its contract. The honest limit is: Runtime spans are exported through the real OTLP SDK when configured; a local collector observed agent.run, but no hosted backend or production sampling was verified.
...
1. Define Tracing and OpenTelemetry without naming an Archon class.
...
1. A trace connects timed spans for one request so operators can see where time and failures occurred. OpenTelemetry supplies standard APIs/export.
2. Traces do not expose chain-of-thought, prove semantic correctness, or replace logs/metrics.
...
4. `backend/app/observability/runtime_events.py:CompositeEventSink.emit` and `backend/tests/unit/test_otel_tracing_wire.py`.
