...
> **Status boundary:** Structlog emits correlation-aware JSON/console events after recursive credential/PII redaction; redaction reduces risk but is not a complete data audit.
...
> **Catalog ID:** `structured-logging`

## Beginner explanation

Structured logs represent an event as named fields rather than prose, making it searchable and correlatable. **It is not:** Logs are operational signals, not durable Run Ledger truth or hidden chain-of-thought.
...
A log is a labeled incident card. The redactor removes sensitive fields before rendering, but classification can never promise all future payloads are safe. Inputs cross a validation boundary; outputs are bounded outcomes plus evidence. Important invariants are explicit scope, finite resources, sanitized failures, and no authority inferred from untrusted payloads.
...
flowchart LR
  ...
  V --> C[Structured logging boundary]
...
| `redact_event` | Implements the central contract. | Production guarantees beyond its wired path. |
...
sequenceDiagram
  ...
  participant Core as redact_event
...
|---|---|---|
| [`backend/app/observability/logging.py:redact_event`](../../../backend/app/observability/logging.py) | Central implemented behavior. | Inspect call sites before extending the claim. |
...
|---|---|---|
| [`backend/tests/unit/test_operational_log_privacy.py`](../../../backend/tests/unit/test_operational_log_privacy.py) | Deterministic contract for the listed implementation. | External-provider parity, load, public deployment, or SLOs. |
...
```bash
cd backend
uv run pytest -q tests/unit/test_operational_log_privacy.py
```

Then locate `redact_event` in `backend/app/observability/logging.py` and write one sentence naming what the test proves and one sentence naming what it cannot prove.

### Done criteria

- [ ] The focused test passes and its assertion is identified.
- [ ] The result is tied to `redact_event`, not merely to a file.
...
> Structured logs represent an event as named fields rather than prose, making it searchable and correlatable. In Archon the boundary is `redact_event`; `backend/tests/unit/test_operational_log_privacy.py` exercises its contract. The honest limit is: Structlog emits correlation-aware JSON/console events after recursive credential/PII redaction; redaction reduces risk but is not a complete data audit.
...
1. Define Structured logging without naming an Archon class.
...
1. Structured logs represent an event as named fields rather than prose, making it searchable and correlatable.
2. Logs are operational signals, not durable Run Ledger truth or hidden chain-of-thought.
...
4. `backend/app/observability/logging.py:redact_event` and `backend/tests/unit/test_operational_log_privacy.py`.
