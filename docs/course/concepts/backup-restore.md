# Concept: Backup and restore

> **Implementation status:** `implemented`
> **Status boundary:** Checksummed PostgreSQL custom dumps and clean-target restore were exercised locally with exact evidence checks; scheduling, remote storage, PITR and cloud failover are absent.
...
> **Catalog ID:** `backup-restore`

## Beginner explanation

A backup is a protected recoverable copy; restore is the tested procedure that turns it back into usable state. A dump without a restore test is only hope. **It is not:** Redis is not durable evidence backup, and encrypted memory also needs key continuity.
...
The checksum seals one artifact against accidental mismatch; it is not encryption or authenticity by itself. Inputs cross a validation boundary; outputs are bounded outcomes plus evidence. Important invariants are explicit scope, finite resources, sanitized failures, and no authority inferred from untrusted payloads.
...
flowchart LR
  ...
  V --> C[Backup and restore boundary]
...
| `pg_dump` | Implements the central contract. | Production guarantees beyond its wired path. |
...
|---|---|---|
| [`scripts/local-backup.sh:pg_dump`](../../../scripts/local-backup.sh) | Central implemented behavior. | Inspect call sites before extending the claim. |
...
Then locate `pg_dump` in `scripts/local-backup.sh` and write one sentence naming what the test proves and one sentence naming what it cannot prove.

### Done criteria

- [ ] The focused test passes and its assertion is identified.
- [ ] The result is tied to `pg_dump`, not merely to a file.
...
> A backup is a protected recoverable copy; restore is the tested procedure that turns it back into usable state. A dump without a restore test is only hope. In Archon the boundary is `pg_dump`; `backend/tests/unit/test_local_dr.py` exercises its contract. The honest limit is: Checksummed PostgreSQL custom dumps and clean-target restore were exercised locally with exact evidence checks; scheduling, remote storage, PITR and cloud failover are absent.
...
1. Define Backup and restore without naming an Archon class.
...
1. A backup is a protected recoverable copy; restore is the tested procedure that turns it back into usable state. A dump without a restore test is only hope.
2. Redis is not durable evidence backup, and encrypted memory also needs key continuity.
...
4. `scripts/local-backup.sh:pg_dump` and `backend/tests/unit/test_local_dr.py`.
