...
| **ReAct** | “Reason and Act”: alternate model decisions, tool actions, and observations until an answer or bounded stop. Cogentrex stores observable actions, not hidden chain-of-thought. | [ReAct](../concepts/react.md) |
...
| **Embedding** | A list of numbers used to compare text similarity. Cogentrex’s current durable store serializes vectors as SQL JSON and computes cosine in Python. | [Embeddings](../concepts/embeddings.md) |
...
| **Application factory** | A function that constructs and consistently configures the application object; callers may inject controlled collaborators. | [Application composition](../concepts/application-composition.md) |
| **Dependency factory** | A callable retained so a concrete dependency can be constructed later, often during startup. | [Application composition](../concepts/application-composition.md) |
| **Middleware** | A wrapper around inbound HTTP requests and outbound responses; ordering changes behavior. | [Application composition](../concepts/application-composition.md) |
| **Router** | A group of related HTTP endpoints registered with the application as one unit. | [Application composition](../concepts/application-composition.md) |
| **Endpoint** | One HTTP method-and-path operation handled by a route function. | [API map](api-map.md) |
| **Application state** | Process-owned shared configuration and services stored on the FastAPI application, not user or conversation memory. | [Application composition](../concepts/application-composition.md) |
...
| **Replay** | Reconstructing a stored trajectory for inspection; in Cogentrex it does not rerun providers or tools. | [Replay/fork/compare](../concepts/replay-fork-compare.md) |
...
| **Self-reflection** | A model critiquing/revising its own reasoning. Cogentrex must not infer generic self-reflection from tool-error feedback, deterministic verification, delegation, or post-run evaluation. |
