...
| Memory | `GET /api/memory/facts`; `GET /api/memory/export`; `DELETE /api/memory/facts`; `GET /api/memory/tiers`; `GET /api/memory/context`; `GET /api/memory/checkpoints` | [`memory.py`](../../../backend/app/routes/memory.py) | Router exists only when encrypted memory is enabled. |
