...
| MCP | `GET /api/mcp/profiles`; `POST, GET /api/mcp/servers`; `GET, PATCH, DELETE /api/mcp/servers/{server_id}`; `POST .../{server_id}/discover`; `GET .../{server_id}/tools`; `PATCH .../{server_id}/tools/{tool_name}` | [`mcp.py`](../../../backend/app/routes/mcp.py) | Deployment-owned stdio and Streamable HTTP profiles expose safe labels only; credentials/configuration are hidden. Discovery is not authorization. Legacy `/request` and `/tools` return `410 Gone`. |
...
| Skills | `GET /api/skills`, `GET /api/skills/catalog`; `POST /api/skills/search`; `POST /api/skills`; `POST /api/skills/install-requests`; `POST /api/skills/{package_id}/revisions/{revision_id}/approve`, `/revoke`; `PUT /api/skills/projects/{project_id}/{package_id}`; `GET /api/skills/projects/{project_id}/effective` | [`skills.py`](../../../backend/app/routes/skills.py) | Exact immutable revisions; create/review operations have declared admin gates, while project binding is owner-scoped. External search returns metadata and does not install content. |
| Project instructions | `POST /api/projects/{project_id}/workspace`; `POST, GET /api/projects/{project_id}/instructions`; `GET .../instructions/revisions`; `POST .../instructions/scan`; `POST .../instructions/{revision_id}/approve`; `POST .../instructions/revoke`; `GET .../instructions/resolve` | [`project_instructions.py`](../../../backend/app/routes/project_instructions.py) | Scan requires a configured authorized workspace root; snapshots are pending until admin approval and exact activation. |
| Capability preferences | `POST /api/capabilities/search`; `PUT /api/capabilities/projects/{project_id}/{capability_id}`; `POST /api/capabilities/projects/{project_id}/effective` | [`capabilities.py`](../../../backend/app/routes/capabilities.py) | Owner/project enable, disable, and pin preferences filter native, skill, and scoped MCP metadata before provider exposure; they do not bypass permission policy. |
