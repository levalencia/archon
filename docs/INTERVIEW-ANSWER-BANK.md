...
On one development Mac run with cached images: backup 0.69 seconds, observed restore-to-ready 24.787 seconds, and zero selected-record differences at the snapshot boundary. Those are reproducibility observations, not production RTO/RPO objectives or SLOs. There is no PITR, remote backup store or multi-region failover evidence.
...
- **Result:** Full backup/destroy/restore passed with exact records/hashes, observed restore-to-ready 24.787 s and zero selected-record differences at the snapshot boundary.
