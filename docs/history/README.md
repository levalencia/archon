# Documentation History

> **Audience:** Maintainers researching how Archon's design and evidence model evolved.
> **Purpose:** Preserve superseded plans, audits, and research without placing them in the current reading path.
> **Status:** Historical index. These documents do not define current implementation status.

Use the [Documentation Index](../README.md) for current guides. Use the [Capability Acceptance manifest](../implementation/CAPABILITY-ACCEPTANCE.yaml) and [Implementation Evidence](../IMPLEMENTATION-EVIDENCE.md) for current technical status.

## Historical plans

- [Original Web Application Plan](PLAN.md)
- [Evidence-First Roadmap V2](ARCHON-EVIDENCE-FIRST-ROADMAP-V2.md)
- [Implementation Status](IMPLEMENTATION-STATUS.md)

## Historical audits

- [GPT-5.6 Re-Audit](ARCHON-GPT56-REAUDIT-2026-08-25.md)
- [Feature and Course Audit V2](FEATURE-AND-COURSE-AUDIT-V2.md)

## Research and strategy

- [Agent God Mode Additions](GOD-MODE-ADDITIONS.md)
- [Matt Pocock Skills Additions](POCOCK-SKILLS-ADDITIONS.md)
- [Launch Playbook](LAUNCH-PLAYBOOK.md)

These files may contain obsolete names, counts, architecture assumptions, or proposed features. Read them as dated context, not as current claims.
