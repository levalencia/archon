...
A superseding Visual Learning Studio acceptance replaced that force-directed overview after direct learner feedback. The current `/learn` experience uses fixed Roadmap, Stories, Architecture, Evidence, Present, Listen, and Study views; every runtime relationship is directional and visibly labeled, and the legacy `/learn/map` route redirects to Stories. The deterministic v2 manifest covers **66 concepts, 16 modules, 5 stories, 5 architecture layers, and 5 NotebookLM notebook recipes**. The source-pack builder emits only allowlisted, tracked public sources outside the repository, records SHA-256 provenance, rejects dirty worktrees/symlinks/credential-like content, and never uploads to Google. The superseding integrated gate passed **1,415 backend tests with 2 expected skips at 87.23% coverage**, Svelte **0 errors and 0 warnings**, **48 Vitest**, the production frontend build, **30 Playwright**, Docker containment/image checks, the deterministic benchmark, and clean-tree verification. These checks prove deterministic generation, runtime wiring, responsive behavior, and source-pack safety; they do not claim that NotebookLM artifacts have already been generated or that learning efficacy has been independently measured.
