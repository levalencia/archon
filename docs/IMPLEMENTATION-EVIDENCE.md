...
| Hybrid agent orchestration pilot | Yes | Yes | Yes | Yes | Yes | No | Feature-flagged Auto/Single/Team routing is wired through sync and SSE. Three paired Foundry comparisons completed without degradation and durable PostgreSQL evidence confirmed three parents, six completed children, lineage, correlation, routing/delegation events, usage and 27 reconciled model charges. Independent review scored Single 27/30 and Team 20/30: Team was slower in every case, tied once and lost twice, so no quality-improvement claim is made. See `docs/evidence/hybrid-agent-live-acceptance-summary.json`. No atomic parent-wide reservation, parallel fan-out, public deployment or production-SLO claim is made. |
...
The feature-flagged hybrid orchestration pilot reuses that signed-envelope boundary for two general-purpose read-only children. `Auto` uses a deterministic versioned heuristic; `Single` preserves the existing canonical path; `Team` creates a fixed researcher plus a server-templated dynamic analyst, each as a real child `AgentRuntime` and durable child run. Per-child runtime budgets override parent defaults, one aggregate deadline bounds the sequential team phase, cancellation records a terminal child event, and findings enter parent synthesis as explicitly delimited untrusted user data under a trusted system guard. Parent events retain only safe allowlisted metadata. Focused TestClient acceptance proves sync/SSE wiring, two provider-backed child executions with the deterministic mock adapter, timeout cancellation, one fixed plus one dynamic profile, parent-child lineage, persisted events, reload reconstruction, and feature-flag rollback. Playwright proves the execution-mode control and Agents inspector against the SSE contract. Current branch gates passed Ruff, 745 marked unit tests, Svelte check with zero diagnostics, the complete Vitest suite, a production frontend build, and the focused Playwright Team flow. Independent backend/security and frontend/evidence reviews identified bounded-budget, cancellation, prompt-authority, lineage, reload, and accessibility defects; those findings were corrected and covered by focused tests. This remains deterministic acceptance, not live-provider quality evidence and not an unbounded or distributed swarm.
